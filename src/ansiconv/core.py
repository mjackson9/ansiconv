...
def _apply_sgr(state: _State, params: List[int], lenient: bool) -> None:
    for code in params:
        if code == _RESET:
            ...
            state.fg = _FG_NAMED[code]
        elif code in _BG_NAMED:
            state.bg = _BG_NAMED[code]
        ...
        # lenient mode: codes we don't model (256-color, blink, strike, ...)
...
def _classes_for(state: _State) -> List[str]:
    ...
    if state.fg:
        classes.append(f"ansi-fg-{state.fg}")
    if state.bg:
        classes.append(f"ansi-bg-{state.bg}")
    return classes
    ...
    while pos < len(text):
        ...
        if m:
            ...
            if not state.is_default():
                out.append(f'<span class="{" ".join(_classes_for(state))}">')
                open_span = True
            pos = m.end()
            continue
...
class _AnsiBuilder(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        ...
        if tag == "span":
            for cls in attrs_dict.get("class", "").split():
                ...
                elif cls.startswith("ansi-fg-") and cls[8:] in _FG_CODE_BY_NAME:
                    new_state.fg = cls[8:]
                elif cls.startswith("ansi-bg-") and cls[8:] in _BG_CODE_BY_NAME:
                    new_state.bg = cls[8:]
    ...
    def _emit_transition(self, new_state: _State) -> None:
        ...
        if new_state.underline:
            codes.append(4)
        if new_state.fg:
            codes.append(_FG_CODE_BY_NAME[new_state.fg])
        if new_state.bg:
            codes.append(_BG_CODE_BY_NAME[new_state.bg])
